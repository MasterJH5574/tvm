...
inline void HIPBLASTryEnableTensorCore(hipblasHandle_t hdl) {
  // TensorCores are only supported in hipblas 9.0 or higher
  int version;
  CHECK_HIPBLAS_ERROR(hipblasLtGetVersion(hdl, &version));
  // TODO
  //   if (version >= 9000) CHECK_HIPBLAS_ERROR(hipblasSetMathMode(hdl, HIPBLAS_DEFAULT_MATH));
}
...
struct HipblasHgemmOp {
  ...
  void operator()(bool ta, bool tb, int M, int N, int K, hipblasHalf alpha, hipblasHalf* A, int lda,
    ...
    alpha = 15360;  // alpha doesn't matter
...
struct HipblasHgemmBatchOp {
  ...
  void operator()(int batch_size, bool ta, bool tb, int M, int N, int K, uint16_t alpha,
                  uint16_t* A, int a_stride, int lda, uint16_t* B, int b_stride, int ldb,
                  uint16_t beta, uint16_t* C, int c_stride, int ldc) {
    alpha = 15360;
    ...
    // CHECK_HIPBLAS_ERROR(hipblasHgemmBatched(handle, HIPBLASBooleanToTranspose(ta),
    //                                         HIPBLASBooleanToTranspose(tb), M, N, K, &alpha, A,
    //                                         lda, B, ldb, &beta, C, ldc, batch_size));
  }
};
...
// inline void CallLtIgemm(TVMArgs args, TVMRetValue* ret, hipblasLtHandle_t hdl, hipStream_t
// stream) {
//   DLTensor* A = args[0];
//   DLTensor* B = args[1];
//   DLTensor* C = args[2];
...
//   hipblasLtOrder_t order_COL32 = HIPBLASLT_ORDER_COL;
//   hipblasLtOrder_t order_COL4_4R2_8C = HIPBLASLT_ORDER_COL;
//   hipblasLtMatmulDesc_t operationDesc = nullptr;
//   // #if CUDART_VERSION >= 11000
//   CHECK_HIPBLAS_ERROR(hipblasLtMatmulDescCreate(&operationDesc, HIPBLAS_COMPUTE_32I, HIP_R_32I));
//   // #else
//   //   CHECK_HIPBLAS_ERROR(hipblasLtMatmulDescCreate(&operationDesc, HIP_R_32I));
//   // #endif
//   hipblasOperation_t opTransA = HIPBLASBooleanToTranspose(transa);
//   hipblasOperation_t opTransB = HIPBLASBooleanToTranspose(transb);
//   CHECK_HIPBLAS_ERROR(hipblasLtMatmulDescSetAttribute(operationDesc,
...
//   CHECK_HIPBLAS_ERROR(hipblasLtMatrixLayoutCreate(
//       &Adesc, HIP_R_8I, opTransA == HIPBLAS_OP_N ? m : k, opTransA == HIPBLAS_OP_N ? k : m,
//       lda));
//   CHECK_HIPBLAS_ERROR(hipblasLtMatrixLayoutCreate(
//       &Bdesc, HIP_R_8I, opTransB == HIPBLAS_OP_N ? k : n, opTransB == HIPBLAS_OP_N ? n : k,
//       ldb));
//   CHECK_HIPBLAS_ERROR(hipblasLtMatrixLayoutCreate(&Cdesc, HIP_R_32I, m, n, ldc));

//   CHECK_HIPBLAS_ERROR(hipblasLtMatrixLayoutSetAttribute(Adesc, HIPBLASLT_MATRIX_LAYOUT_ORDER,
//                                                         &order_COL32, sizeof(order_COL32)));
//   CHECK_HIPBLAS_ERROR(hipblasLtMatrixLayoutSetAttribute(
//       Bdesc, HIPBLASLT_MATRIX_LAYOUT_ORDER, &order_COL4_4R2_8C, sizeof(order_COL4_4R2_8C)));
//   CHECK_HIPBLAS_ERROR(hipblasLtMatrixLayoutSetAttribute(Cdesc, HIPBLASLT_MATRIX_LAYOUT_ORDER,
//                                                         &order_COL32, sizeof(order_COL32)));

//   CHECK_HIPBLAS_ERROR(hipblasLtMatmul(hdl, operationDesc, &alpha, B_data, Adesc, A_data, Bdesc,
//                                       &beta, C_data, Cdesc, C_data, Cdesc, nullptr, nullptr, 0,
//                                       stream));
// }
...
TVM_REGISTER_GLOBAL("tvm.contrib.hipblas.matmul").set_body([](TVMArgs args, TVMRetValue* ret) {
  ...
  HIPBLASTryEnableTensorCore(entry_ptr->handle);
...
// TVM_REGISTER_GLOBAL("tvm.contrib.hipblaslt.matmul").set_body([](TVMArgs args, TVMRetValue* ret) {
//   DLTensor* A = args[0];
...
//   CHECK_HIPBLAS_ERROR(hipblasLtCreate(&ltHandle));
...
//   CHECK_HIPBLAS_ERROR(hipblasLtDestroy(ltHandle));
// });
