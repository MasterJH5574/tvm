...
#include "groupwise_scaled_gemm_runner_sm100.cuh"
...
void tvm_cutlass_fp8_blockwise_scaled_gemm_sm100(NDArray a, NDArray b, NDArray scales_a,
  ...
  if (DataType(out->dtype) == DataType::Float(16)) {
    cutlass_fp8_blockwise_scaled_mm_sm100<TileShape, ClusterShape, cutlass::float_e4m3_t,
        ...
        workspace->shape[0] * DataType(workspace->dtype).bytes(), m, n, k, /*l=*/1, stream);
...
void tvm_cutlass_fp8_blockwise_scaled_bmm_sm100(NDArray a, NDArray b, NDArray scales_a,
                                                NDArray scales_b, NDArray workspace,
                                                int64_t block_size_0, int64_t block_size_1,
                                                NDArray out) {
  using TileShape = Shape<_128, _128, _128>;
...
TVM_REGISTER_GLOBAL("cutlass.blockwise_scaled_gemm_e4m3fn_e4m3fn_sm100")
    .set_body_typed(tvm_cutlass_fp8_blockwise_scaled_gemm_sm100);
TVM_REGISTER_GLOBAL("cutlass.blockwise_scaled_bmm_e4m3fn_e4m3fn_sm100")
    .set_body_typed(tvm_cutlass_fp8_blockwise_scaled_bmm_sm100);
