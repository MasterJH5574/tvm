...
class TorchFXImporter:
    ...
    def _convert_data_type(
        input_type: Union[str, torch.dtype], env: Optional[Dict] = None
    ):
    ...
    def _gelu(self, node: fx.Node) -> relax.Expr:
        ...
        if approximate == "none":
            ...
            return self.block_builder.emit(
                relax.op.nn.gelu_tanh(self.env[node.args[0]])
            )
        else:
            raise KeyError(
                "Unregonized approximate algorithm for gelu: {}.".format(approximate)
            )
    ...
    def _addmm(self, node: fx.Node) -> relax.Var:
        ...
        if beta != 0:
            ...
            res = (
                bias
                if res is None
                else self.block_builder.emit(relax.op.add(bias, res))
            )
        return res
    ...
    def _einsum(self, node: fx.Node) -> relax.Var:
        ...
        args = self.retrieve_args(node)
        operands = (
            args[1] if isinstance(args[1], (torch.Size, tuple, list)) else args[1:]
        )
        ...
        return self.block_builder.emit(
            transpose_S_H(
                relax.op.nn.attention(
                    query, key, value, bias=attn_mask, causal_mask=causal_mask
                )
            )
        )
    ...
    def _getitem(self, node: fx.Node) -> relax.Var:
        ...
        if isinstance(x, (list, tuple, relax.ShapeExpr, relax.Tuple)):
            ...
            for each_index, each_axis in zip(take_indices, take_axes):
                taken = self.block_builder.emit(
                    relax.op.take(taken, each_index, each_axis)
                )
            sliced = self.block_builder.emit(
                relax.op.strided_slice(
                    taken, stride_axes, stride_begin, stride_end, stride
                )
            )
    ...
    def _flatten(self, node: fx.Node) -> relax.Var:
        x = self.env[node.args[0]]
        start_dim = (
            node.args[1] if len(node.args) >= 2 else node.kwargs.get("start_dim", 0)
        )
        end_dim = (
            node.args[2] if len(node.args) == 3 else node.kwargs.get("end_dim", -1)
        )
    ...
    def _full(self, node: fx.Node) -> relax.Var:
        ...
        value = (
            args[1]
            if isinstance(args[1], relax.expr.Constant)
            else relax.const(args[1], dtype)
        )
        return self.block_builder.emit(
            relax.op.full(
                size,
    ...
    def _masked_scatter(self, node: fx.Node) -> relax.Var:
        ...
        if ndim == 1:
            ...
            index = self.block_builder.emit(
                relax.op.subtract(index, relax.const(1, "int32"))
            )
            gathered_source = self.block_builder.emit(
                relax.op.take(source, index, axis=0)
            )
        else:
            ...
            gathered_source = self.block_builder.emit(
                relax.op.take(f_source, index, axis=0)
            )
            gathered_source = self.block_builder.emit(
                relax.op.reshape(gathered_source, x.struct_info.shape)
            )
        if ndim != len(x.struct_info.shape):
            mask = self.block_builder.emit(
                relax.op.broadcast_to(mask, x.struct_info.shape)
            )
    ...
    def create_input_vars(
        self, input_info: List[Tuple[Tuple[int], str]]
    ) -> List[relax.Var]:
        inputs = list()
                ...
                for _, param in model.named_parameters():
                    ...
                    if dtype in ("float32", "float16"):
                        if not keep_params_as_input:
                            self.params[param] = self._convert_torch_tensor_to_relax(
                                param
                            )
                    else:
                        raise ValueError(
