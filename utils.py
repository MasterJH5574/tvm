...
def onnx_backend(gm: torch.fx.GraphModule, example_inputs: List[torch.Tensor]):
    ...
    real_inputs = tuple(
        [
            torch.rand(x.shape, dtype=x.dtype, layout=x.layout, device=x.device)
    ...
    def fn(*args):
        ort_inputs = {
            onnx_input_names[i]: args[i].contiguous().cpu().detach().numpy()
...
def assert_equal(ref, out):
    precision = 5e-3
    assert type(ref) == type(out), f"wrong type: expect {type(ref)}, got {type(out)}"
    ...
    def _relax_backend(graph_module, example_inputs):
        ...
        def to_torch_tensor(nd_tensor):
            ...
            if isinstance(nd_tensor, tvm.nd.NDArray):
                return dlpack.from_dlpack(nd_tensor.to_dlpack())
...
def perf_test(
    ...
    if compile_mode == "trace":
        ...
        def fn(*args):
            fx_graph = torch.fx.symbolic_trace(
                f
            )  # intentionally retrace for time measure
        ...
        #     sys_config.set_config("debug", False)
        #     sys_config.set_config("backend", backend)
        #     compiled = sys_compile(f)
    else:
    ...
    if dynamic_mode == "cf":
        ...
        if compile_mode == "sys-dynamic":
            with enable_dyn_shape():
                perf_test_run_seq_len(
                    f, compiled, compile_mode, num_repeat, get_input_fn
                )
        else:
            perf_test_run_seq_len(f, compiled, compile_mode, num_repeat, get_input_fn)
    elif compile_mode == "sys-profile":
        perf_test_with_profile(
            compiled, graph_timer, compile_mode, num_repeat, args, kwargs
        )
    else:
...
def read_bin(s, dtype=np.float32):
    with open(s + ".shape") as f:
        shape = tuple((int(x) for x in f.read().strip().split(" ")))
...
def save_bin(data, path):
    ...
    with open(path + ".shape", "w") as f:
        f.write(" ".join(str(x) for x in data.shape))
