...
with NO_LD_PRELOAD_CTX():
    ...
    def main():
        ...
        if args.dyn_cf:
            ...
            input_args, input_kwargs = module.get_dynamic_inputs(
                args.bs, 2 * args.repeat
            )
            if args.model == "blockdrop":
                frontend.dynamic.add_branch_rewrite_pc(
                    frontend.c_api.get_next_frame_id(), 51
                )
            if args.model == "lstm":
                for_iter_pc = 32
                frontend.dynamic.mark_dynamic_pc(
                    frontend.c_api.get_next_frame_id(),
                    for_iter_pc,
