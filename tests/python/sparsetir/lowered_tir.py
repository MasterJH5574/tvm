...
@T.prim_func
def lowered_csrmm(a: T.handle, b: T.handle, c: T.handle, indptr: T.handle, indices: T.handle, m: T.int32, n: T.int32, k: T.int32, nnz: T.int32) -> None:
    # function attr dict
    ...
    for v_vi, v_vk in T.grid(m, k):
        with T.block("csrmm0"):
            ...
            T.reads(J_indptr[0: m + 1], J_indices[0: nnz],
                    A_data[0: nnz], B_data[0: n * k], C_data[0: m * k])
...
@T.prim_func
def lowered_csrmm_dense_iter(a: T.handle, b: T.handle, c: T.handle, indptr: T.handle, indices: T.handle, m: T.int32, n: T.int32, k: T.int32, nnz: T.int32) -> None:
    # function attr dict
    T.func_attr({"global_symbol": "main", "tir.noalias": True})
    A_data = T.match_buffer(a, (nnz,), "float32")
    B_data = T.match_buffer(b, (n * k,), "float32")
    ...
    for v_vi, v_vj, v_vk in T.grid(m, n, k):
        with T.block("csrmm0"):
            ...
            with T.init():
                C_data[vi * k + vk] = T.float32(0)
            C_data[vi * k + vk] = C_data[vi * k + vk] + A_data[T.tvm_lower_bound(
                J_indices.data, vj, J_indptr[vi], J_indptr[vi + 1], dtype="int32")] * B_data[vj * k + vk]
...
@T.prim_func
def lowered_segment_reduce(a: T.handle, b: T.handle, indptr: T.handle, n: T.int32, nnz: T.int32) -> None:
    A_data = T.match_buffer(a, (nnz,), "float32")
...
@T.prim_func
def lowered_bsrmm(a: T.handle, b: T.handle, c: T.handle, j_indptr: T.handle, j_indices: T.handle, nb: T.int32, mb: T.int32, nnzb: T.int32, blk: T.int32, feat_size: T.int32) -> None:
...
@T.prim_func
def lowered_ellmm(a: T.handle, b: T.handle, c: T.handle, j_indices: T.handle, nb: T.int32, mb: T.int32, feat_size: T.int32, col: T.int32, blk: T.int32) -> None:
    ...
    for v_vi, v_vj, v_vbi, v_vbj, v_vf in T.grid(nb, col, blk, blk, feat_size):
        with T.block("ellmm0"):
            ...
            T.reads(J_indices[0: nb * col], A_data[0: nb * col * blk * blk],
                    B_data[0: mb * blk * feat_size], C_data[0: nb * blk * feat_size])
            T.writes(C_data[0: nb * blk * feat_size])
            T.block_attr({"sparse": True})
            with T.init():
...
@T.prim_func
def lowered_sddmm(a: T.handle, b: T.handle, c: T.handle, indptr: T.handle, indices: T.handle, m: T.int32, n: T.int32, k: T.int32, nnz: T.int32) -> None:
    T.func_attr({"global_symbol": "main", "tir.noalias": True})
    ...
    for v_vi in T.serial(m):
        with T.block("sddmm0"):
            ...
            T.reads(J_indptr[0: m + 1], J_indices[0: nnz],
                    A_data[0: m * k], B_data[0: n * k], C_data[0: nnz])
            ...
            for v_vj, v_vk in T.grid(J_indptr[vi + 1] - J_indptr[vi], k):
                with T.block("sddmm1"):
                    ...
                    C_data[J_indptr[vi] + vj] = C_data[J_indptr[vi] + vj] + \
                        A_data[vi * k + vk] * B_data[J_indices[J_indptr[vi] + vj] * k + vk]


# from tvm.script import tir as T
@T.prim_func
def lowered_sddmm_fuse(a: T.handle, b: T.handle, c: T.handle, indptr: T.handle, indices: T.handle, m: T.int32, n: T.int32, k: T.int32, nnz: T.int32) -> None:
    T.func_attr({"global_symbol": "main", "tir.noalias": True})
    A_data = T.match_buffer(a, (m * k,), "float32")
    B_data = T.match_buffer(b, (n * k,), "float32")
    C_data = T.match_buffer(c, (nnz,), "float32")
    ...
    for v_vi, v_vj, v_vk in T.grid(1, nnz, k):
        with T.block("sddmm0"):
            ...
            C_data[vj] = C_data[vj] + A_data[(T.tvm_upper_bound(J_indptr.data, vj, 0,
                                              m + 1, dtype="int32") - 1) * k + vk] * B_data[J_indices[vj] * k + vk]
...
def lowered_bmm(
    ...
    for v_vb in T.serial(batch_size):
        with T.block("bmm0"):
            ...
            T.reads(I_indptr[0: batch_size + 1], J_indptr[0: batch_size + 1], K_indptr[0: batch_size + 1], IJ_indptr[0: batch_size + 1],
                    JK_indptr[0: batch_size + 1], IK_indptr[0: batch_size + 1], X_data[0: nnz_ij], Y_data[0: nnz_jk], Z_data[0: nnz_ik])
            T.writes(Z_data[0: nnz_ik])
            T.block_attr({"sparse": True})
            for v_vi, v_vj, v_vk in T.grid(I_indptr[vb + 1] - I_indptr[vb], J_indptr[vb + 1] - J_indptr[vb], K_indptr[vb + 1] - K_indptr[vb]):
                with T.block("bmm1"):
                    vi, vj, vk = T.axis.remap("SRS", [v_vi, v_vj, v_vk])
                    T.reads(I_indptr[0: batch_size + 1], J_indptr[0: batch_size + 1], K_indptr[0: batch_size + 1], IJ_indptr[0: batch_size + 1],
                            JK_indptr[0: batch_size + 1], IK_indptr[0: batch_size + 1], X_data[0: nnz_ij], Y_data[0: nnz_jk], Z_data[0: nnz_ik])
                    ...
                    T.block_attr({"sparse": True})
                    with T.init():
                        Z_data[IK_indptr[vb] + vi *
                               (K_indptr[vb + 1] - K_indptr[vb]) + vk] = T.float32(0)
                    Z_data[IK_indptr[vb] + vi * (K_indptr[vb + 1] - K_indptr[vb]) + vk] = Z_data[IK_indptr[vb] + vi * (K_indptr[vb + 1] - K_indptr[vb]) + vk] + \
...
def lowered_square_sum(a: T.handle, b: T.handle, indptr_j: T.handle, indices_j: T.handle, indptr_k: T.handle, indices_k: T.handle, nnz_j: T.int32, nnz_k: T.int32, M: T.int32, N1: T.int32, N2: T.int32) -> None:
    ...
    for v_vi in T.serial(0, M):
        with T.block("square_sum_2"):
            vi = T.axis.spatial(M, v_vi)
            T.reads([J_indptr[0: M + 1], J_indices[0: nnz_j], K_indptr[0: nnz_j + 1],
                    K_indices[0: nnz_k], A_data[0: nnz_k], B_data[0: M]])
            T.writes([B_data[0: M]])
            T.block_attr({"sparse": True})
            for v_vj in T.serial(0, J_indptr[vi + 1] - J_indptr[vi]):
                with T.block("square_sum_1"):
                    ...
                    with T.init():
                        B_data[vi] = T.float32(0)
                    for v_vk in T.serial(0, K_indptr[J_indptr[vi] + vj + 1] - K_indptr[J_indptr[vi] + vj]):
...
def lowered_square_sum_two_K(a: T.handle, b: T.handle, indptr_j: T.handle, indices_j: T.handle, indptr_k0: T.handle, indices_k0: T.handle, indptr_k1: T.handle, indices_k1: T.handle, nnz_j: T.int32, nnz_k: T.int32, M: T.int32, N1: T.int32, N2: T.int32) -> None:
    ...
    for v_vi in T.serial(0, M):
        with T.block("square_sum_2"):
            vi = T.axis.spatial(M, v_vi)
            T.reads([J_indptr[0: M + 1], J_indices[0: nnz_j], K0_indptr[0: nnz_j + 1], K0_indices[0: nnz_k],
                    K1_indptr[0: nnz_j + 1], K1_indices[0: nnz_k], A_data[0: nnz_k], B_data[0: M]])
            T.writes([B_data[0: M]])
            T.block_attr({"sparse": True})
            for v_vj in T.serial(0, J_indptr[vi + 1] - J_indptr[vi]):
                with T.block("square_sum_1"):
                    ...
                    with T.init():
                        B_data[vi] = T.float32(0)
                    for v_vk in T.serial(0, K1_indptr[J_indptr[vi] + vj + 1] - K1_indptr[J_indptr[vi] + vj]):
                        with T.block("square_sum"):
                            vk = T.axis.reduce(
                                K1_indptr[J_indptr[vi] + vj + 1] - K1_indptr[J_indptr[vi] + vj], v_vk)
                            T.reads([J_indptr[0: M + 1], J_indices[0: nnz_j], K0_indptr[0: nnz_j + 1], K0_indices[0: nnz_k],
                                    K1_indptr[0: nnz_j + 1], K1_indices[0: nnz_k], A_data[0: nnz_k], B_data[0: M]])
                            T.writes([B_data[0: M]])
                            T.block_attr({"sparse": True})
                            B_data[vi] = B_data[vi] + A_data[T.tvm_lower_bound(
                                K0_indices.data, K1_indices[K1_indptr[J_indptr[vi] + vj] + vk], K0_indptr[J_indptr[vi] + vj], K0_indptr[J_indptr[vi] + vj + 1], dtype="int32")]
...
@T.prim_func
def lowered_rgcn_forward(etype: T.handle, w: T.handle, x: T.handle, y: T.handle, indptr: T.handle, indices: T.handle, n: T.int32, r: T.int32, feat_size: T.int32, nnz: T.int32) -> None:
