...
def test_bmm():
    ...
    # specialize function
    _, _, _, _, _, _, _, _, _, BATCH, NNZ_I, NNZ_J, NNZ_K, NNZ_IJ, NNZ_JK, NNZ_IK = lowered_bmm.params
    sch = tir.Schedule(
        lowered_bmm.specialize({
            BATCH: batch_size, NNZ_I: nnz_i, NNZ_J: nnz_j, NNZ_K: nnz_k, NNZ_IJ: nnz_ij, NNZ_JK: nnz_jk, NNZ_IK: nnz_ik
        })
    )
...
def test_square_sum():
    ...
    f = tvm.build(lowered_square_sum.specialize(
        {v_nnz_j: nnz_j, v_nnz_k: nnz_k, v_M: M, v_N1: N1, v_N2: N2}), target="llvm")
...
def test_square_sum_two_K():
    ...
    f = tvm.build(sch.mod["main"].specialize(
        {v_nnz_j: nnz_j, v_nnz_k: nnz_k, v_M: M, v_N1: N1, v_N2: N2}), target="cuda")
