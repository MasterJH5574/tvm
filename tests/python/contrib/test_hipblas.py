...
from tvm.contrib import hipblaslt
...
#     C = hipblaslt.matmul(A, B, False, True, m, N_out, dtype=out_dtype)
#     s = te.create_schedule(C.op)

#     def verify(target="cuda"):
#         if not tvm.get_global_func("tvm.contrib.hipblaslt.matmul", True):
#             print("skip because extern function is not available")
#             return
#         dev = tvm.cuda(0)
#         f = tvm.build(s, [A, B, C], target)
...
def verify_batch_matmul(Ashape, Bshape, Cshape, in_dtype, out_dtype, rtol=1e-5):
    ...
    print(a.asnumpy()[0][0][0])
    print(b.asnumpy()[0][0][0])
    f(a, b, c)
    tvm.testing.assert_allclose(
        c.numpy(),
